...
export default function OSDetail() {
  ...
  // Mecânico (ou gerente) pode "pegar" se a OS está REGISTADA e ainda não tem dono
  const canPegar =
    (role === "MECANICO" || role === "GERENTE") &&
    !!user && !os.mecanicoId && os.estado === "REGISTADA";
    ...
    <div>
      ...
      <Tabs defaultValue="resumo" className="space-y-4">
        <TabsList>
          ...
          <TabsTrigger value="reparacao">Conserto</TabsTrigger>
        ...
        <TabsContent value="resumo">
          <ResumoTab
            ...
            mecanicos={mecanicos}
            criador={todosFuncionarios.find((f) => f.id === os.criadaPor) ?? null}
            canPegar={canPegar}
...
// ---------- Resumo ----------
function ResumoTab({
  os, cliente, trotinete, mecanicos, criador, canPegar, bloqueadoOutroMec, onPegar,
}: {
      ...
      <Card>
        ...
        <CardContent className="space-y-3 text-sm">
          <div>
            ...
            <p className="mt-1">{criador ? criador.nome : "—"}</p>
          </div>
          <div>
              ...
              <TableBody>
                {reps.map((ir) => {
                  ...
                  const toggle = () => {
                    if (!canEdit) return;
                    setReps((p) => p.map((x) =>
                      x.reparacaoId === ir.reparacaoId ? { ...x, feita: !x.feita } : x));
                  };
                  return (
                    <TableRow
                      key={ir.reparacaoId}
            ...
            <Table>
              ...
              <TableBody>
                {pcs.map((ip) => {
                  ...
                  const podeToggle = canEdit && !semStock;
                  const toggle = () => {
                    if (!podeToggle) return;
                    setPcs((arr) => arr.map((x) =>
                      x.pecaId === ip.pecaId ? { ...x, usada: !x.usada } : x));
                  };
                  return (
                    <TableRow
                      key={ip.pecaId}
                      onClick={podeToggle ? toggle : undefined}
                      className={podeToggle ? "cursor-pointer" : undefined}
                    >
                      <TableCell onClick={(e) => e.stopPropagation()}>
                        <Checkbox
                          checked={!!ip.usada}
