...
export const funcionariosService = {
  ...
  async resetHorasExtra(funcionarioId: string): Promise<void> {
    await delay();
    const db = getDB();
    const f = db.funcionarios.find((x) => x.id === funcionarioId);
    if (f) f.horasExtraAcumuladas = 0;
    db.horasExtra = db.horasExtra.filter((h) => h.funcionarioId !== funcionarioId);
    persist();
  },
};
...
export const entradasService = {
  ...
  async remove(id: string): Promise<void> {
    await delay();
    const db = getDB();
    const ent = db.entradas.find((e) => e.id === id);
    if (!ent) return;
    const peca = db.pecas.find((p) => p.id === ent.pecaId);
    if (peca) peca.stockAtual = Math.max(0, peca.stockAtual - ent.quantidade);
    db.entradas = db.entradas.filter((e) => e.id !== id);
    db.movimentos = db.movimentos.filter((m) => m.refId !== id);
    persist();
  },
};
